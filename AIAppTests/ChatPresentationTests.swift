...
final class ChatPresentationTests: XCTestCase {
    ...
    func testComposerUsesUnifiedPlusMenuAndInlineDictation() throws {
        ...
        let photo = try XCTUnwrap(source.range(of: "Label(\"Foto\", systemImage: \"photo\")"))
        let camera = try XCTUnwrap(source.range(of: "Label(\"Kamera\", systemImage: \"camera\")"))
        let file = try XCTUnwrap(source.range(of: "Label(\"Datei\", systemImage: \"doc\")"))
        XCTAssertLessThan(photo.lowerBound, camera.lowerBound)
        XCTAssertLessThan(camera.lowerBound, file.lowerBound)
