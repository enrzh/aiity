...
final class ProbeStubServer {
    ...
    private func respond(_ connection: NWConnection, method: String, path: String) {
        let (status, body) = route(method: method, path: path)
    ...
    private func route(method: String, path: String) -> (Int, Data) {
        func json(_ text: String) -> Data { Data(text.utf8) }
        let miss = (404, json(#"{"error":{"message":"not found"}}"#))
        ...
        case .openai:
            if method == "GET", path.hasPrefix("/v1/models") || path.hasSuffix("/models") {
                return (200, json(#"{"data":[{"id":"stub-large","object":"model"},{"id":"stub-mini","object":"model"}]}"#))
            }
            if method == "POST", path.hasSuffix("/chat/completions") {
                return (200, json(#"{"id":"chatcmpl-stub","object":"chat.completion","choices":[{"index":0,"finish_reason":"stop","message":{"role":"assistant","content":"ok"}}]}"#))
            }
        case .anthropic:
            if method == "GET", path.hasPrefix("/v1/models") {
                return (200, json(#"{"data":[{"type":"model","id":"claude-stub-1","display_name":"Claude Stub"}],"has_more":false}"#))
            }
            if method == "POST", path.hasSuffix("/messages") {
                return (200, json(#"{"id":"msg_stub","type":"message","role":"assistant","content":[{"type":"text","text":"ok"}],"stop_reason":"end_turn"}"#))
            }
        case .ollamaNative:
            if method == "GET", path.hasPrefix("/api/tags") {
                return (200, json(#"{"models":[{"name":"qwen2.5:0.5b","model":"qwen2.5:0.5b"}]}"#))
            }
            if method == "POST", path.hasSuffix("/chat/completions") {
                return (200, json(#"{"choices":[{"index":0,"finish_reason":"stop","message":{"role":"assistant","content":"ok"}}]}"#))
            }
        }
        return miss
    }
